...
/**
 * True when the visitor has objected to usage analytics. We honour an explicit
 * opt-out from the UI (`X-WebIQ-Analytics: off`) as well as the browser-level
 * Do Not Track (`DNT: 1`) and Global Privacy Control (`Sec-GPC: 1`) signals.
 * When opted out, callers skip the pseudonymous id and all custom telemetry.
 */
export function analyticsOptedOut(req: Request): boolean {
  const optOutHeader = req.headers['x-webiq-analytics'];
  const optOut = Array.isArray(optOutHeader) ? optOutHeader[0] : optOutHeader;
  if (optOut?.toLowerCase() === 'off') {
    return true;
  }

  const dnt = req.headers['dnt'];
  if ((Array.isArray(dnt) ? dnt[0] : dnt) === '1') {
    return true;
  }

  const gpc = req.headers['sec-gpc'];
  return (Array.isArray(gpc) ? gpc[0] : gpc) === '1';
}
