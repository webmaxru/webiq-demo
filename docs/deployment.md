...
- **Web IQ key rotation:** the encrypted `WEBIQ_API_KEY` repository secret is validated
  in the candidate image before Azure is changed, synced into ACA before every revision,
  and verified against the deployed backend. Keep it synchronized with `.env` and the
  azd environment value used by Bicep.
