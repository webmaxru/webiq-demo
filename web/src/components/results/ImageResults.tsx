  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {images.map((item, index) => {
        ...
        return (
          <a className="group overflow-hidden rounded-2xl border border-ink-200 bg-white" href={href} key={`${href ?? index}`} rel="noreferrer" target="_blank">
            ...
            <div className="space-y-1 p-3">
              <h3 className="line-clamp-2 text-sm font-semibold text-ink-950 group-hover:text-brand-600">{asString(record.title) ?? 'Image result'}</h3>
              <p className="line-clamp-2 text-xs text-ink-600">{asString(record.caption)}</p>
